...
from mongo.helpers import bulk
...
def init_mongo(uri):
    # init mongo
    es_conn = mongo.mongo(uri)
    try:
        es_conn.info()
    except mongo.ConnectionError:
        raise mongoConnectionError(uri)
...
def mongo_batch_restreamer(dataframe, timefield, es_conn, index_name,
                                   interval=10, first_pass=True):
    """
    Replay input stream into mongo
    """
